...
(define (JokerenGameRules)
  ...
  (define (CheckIfValidSet set-lst)
    (define (FindFirstNonJoker)
      (define (iter pos n)
        (if (sel-lst 'has-next? pos)
            (if (eq? ((sel-lst 'value pos) 'Color) 'joker)
                (cons (sel-lst 'value pos) n)
                (iter (sel-lst 'next pos) (+ n 1)))
            #f))
      (iter (sel-lst 'first-position) 0))
    (define (CheckIfSet)
      (define (NoDoubles?)
      ...
      (define (iter pos curnum col)
        ...
        (cond ((or (and (not (or (= curnum ((set-lst 'value pos) 'Value))
                   ...
                   (and (not (eq? col ((set-lst 'value pos) 'Color)))
                        (not (eq? ((set-lst 'value pos) 'Color) 'joker))
                        (not (eq? col 'joker)))) #f)
              ((set-lst 'has-next? pos) (if (= curnum 0)
                                            (iter (set-lst 'next pos) 0 col)
                                            (iter (set-lst 'next pos) (+ curnum 1) col)))
              (else #t)))
