  ...
  (define (Recalculate)
    (let ((poslst (table 'toPosList)))
      (define (NextPos pos x y)
        (define cardWidth ((poslst 'value pos) 'GUIWidth guidesc))
        (set! x (+ x cardWidth))
  ...
  (define (find-container c-posn posnlst poslst)
    (define (find-x pos tbpos)
      ...
      (if (poscmph? >= c-posn (pos 'value)) 
          (if (poscmph? >= c-posn (make-posn (+ (posn-x (pos 'value)) ((tbpos 'value) 'GUIWidth guidesc)) (posn-y (pos 'value))))
              (if (pos 'has-next?)
    ...
    (define (iter pos tbpos)
      (if (poscmpv? >= c-posn (pos 'value)) 
          (if (poscmpv? >= c-posn (make-posn (posn-y (pos 'value)) (+ (posn-y (pos 'value)) ((tbpos 'value) 'GUIHeight guidesc))))
              (if (pos 'has-next?)
