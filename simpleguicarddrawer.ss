...
(define (SimpleGUICardDrawer vp)
  ...
  (define (FindCardOffset cardlst c-posn containerpos maxevosize evo )
    (define (CalcHorizSpace)
      (min (quotient (- maxevosize 71) (- (lst 'length) 1))
           14))
    (define (CalcVertSpace)
      (min (quotient (- maxevosize 96) (- (lst 'length) 1))
           20))
    (define (calc-x)
      (let ((res (quotient (- (posn-x c-posn) (posn-x containerpos)) 14)))
        (if (> res (- (cardlst 'length) 1))
            (if (< (- (posn-x c-posn) (+ (* (- (cardlst 'length) 1) 14) (posn-x containerpos))) 71)
                (- (cardlst 'length) 1)
                #f)
            res)))
    (define (calc-y)
      (let ((res (quotient (- (posn-x c-posn) (posn-x containerpos)) 20)))
        (if (> res (- (cardlst 'length) 1))
            (if (< (- (posn-y c-posn) (+ (* (- (cardlst 'length) 1) 20) (posn-y containerpos))) 96)
                (- (cardlst 'length) 1)
                #f)
            res)))
        (if (eq? evo 'horizontal)
            (calc-x)
            (calc-y)))
  ...
  (λ msg
    (if (null? msg)
        ...
        (case (car msg)
          ...
          ('FindCardOffset (FindCardOffset (GetParam msg 0) (GetParam msg 1) (GetParam msg 2) (GetParam msg 3) (GetParam msg 4)))
